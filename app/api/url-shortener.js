...
module.exports = function(app, db) {
  ...
  function handleGet(req, res) {
    // gets the url from the database and redirects if it exists
    var url = req.params.url + req.params['0'];
    if (url != process.env.OG_URL + 'favicon.ico') {
      findURL(url, db, res);
    }
  }
  ...
  function linkGen(url, res, db, callback) {
    ...
    db.collection('sites').find().toArray((err, data) => {
      // .find() selects all documents in collection
      ...
      do {
        ...
        newLink = process.env.OG_URL + num.toString().substring(0, 4);
  ...
  function findURL(link, db, res) {
    console.log("Searching for existing URL")
    console.log(link)
    // Check to see if the site (link) is already there
    var sites = db.collection('sites');
    // get the url
    sites.findOne({
      "original_url": link
